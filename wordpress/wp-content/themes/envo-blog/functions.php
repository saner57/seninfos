		...
		if ( $categories_list || $tags_list || get_edit_post_link() ) {
			...
			if ( 'post' === get_post_type() ) {
				if ( $categories_list || $tags_list ) {
					...
					if ( $categories_list ) {
						echo '<div class="cat-links"><span class="space-right">' . esc_html__( 'Category', 'envo-blog' ) . '</span>' . $categories_list . '</div>';
					}

					if ( $tags_list ) {
						echo '<div class="tags-links"><span class="space-right">' . esc_html__( 'Tags', 'envo-blog' ) . '</span>' . $tags_list . '</div>';
					}
				}
			}
...
if ( !function_exists( 'envo_blog_generate_construct_footer' ) ) :
	...
	function envo_blog_generate_construct_footer() {
		...
		<p class="footer-credits-text text-center">
			<?php printf( __( 'Proudly powered by %s', 'envo-blog' ), '<a href="' . esc_url( __( 'https://wordpress.org/', 'envo-blog' ) ) . '">WordPress</a>' ); ?>
			<span class="sep"> | </span>
			<?php printf( __( 'Theme: %1$s', 'envo-blog' ), '<a href="http://envothemes.com/">Envo Blog</a>' ); ?>
		</p> 
		<?php
	}
